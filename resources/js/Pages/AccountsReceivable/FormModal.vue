...
watch(() => props.visible, (val) => {
    if (val) {
        if (props.account) {
            form.customer_id = props.account.customer_id ?? null;
...
function submit() {
    if (props.account) {
        form.put(route('accounts-receivable.update', props.account.id), { onSuccess: () => close() });
    } else {
        form.post(route('accounts-receivable.store'), { onSuccess: () => close() });
    }
}
</script>
...
<template>
    <Dialog :visible="visible" @update:visible="$emit('update:visible', $event)"
        :header="account ? 'Editar Conta a Receber' : 'Nova Conta a Receber'"
