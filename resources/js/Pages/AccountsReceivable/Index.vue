...
const props = defineProps({ accounts: Object, filters: Object, customers: Array, statuses: Array });
...
function applyFilters() {
    router.get(route('accounts-receivable.index'), { search: search.value, status: status.value }, { preserveState: true, replace: true });
}
...
function deleteAccount() { router.delete(route('accounts-receivable.destroy', deleteId.value)); confirmDelete.value = false; }
function markReceived(id) { router.post(route('accounts-receivable.receive', id)); }
</script>

<template>
    <ErpLayout title="Contas a Receber">
        <template #header><h2 class="text-lg font-semibold text-gray-800">Contas a Receber</h2></template>

        <BasePageHeader title="Contas a Receber" create-label="Nova Conta" @new="openCreate" />

        <div class="bg-white rounded-xl border border-gray-200">
            <div class="p-4 border-b border-gray-100 flex flex-wrap gap-3 items-center">
                <input v-model="search" type="text" placeholder="Buscar por descrição ou cliente..." class="w-72 rounded-lg border-gray-300 text-sm" />
                ...
                <Column header="Ações" style="width: 160px">
                    <template #body="{ data }">
                        <div class="flex gap-1">
                            <button v-if="data.status === 'pending'" @click="markReceived(data.id)"
                                    class="px-2 py-1 text-xs bg-green-100 text-green-700 rounded hover:bg-green-200" title="Marcar como Recebido">
                                <i class="pi pi-check"></i> Receber
                            </button>
        ...
        <FormModal v-model:visible="modalVisible" :account="editingAccount" :customers="customers" :statuses="statuses" />
        <BaseConfirmDialog v-model:visible="confirmDelete" title="Excluir Conta" message="Tem certeza que deseja excluir esta conta a receber?" @confirm="deleteAccount" />
    </ErpLayout>
</template>
